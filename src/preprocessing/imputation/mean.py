...
def replace_missing_values(train, test):
    ...
    # Replace zero and negative values with NaN to prevent them from affecting mean calculation
    ...
    print("-------Dữ liệu huấn luyện bị thiếu:")
    print(missing_values)

    # Separate the data based on Outcome for different mean calculations
    train_diabetic = train[train['Outcome'] == 1]
    train_non_diabetic = train[train['Outcome'] == 0]

    # Calculate mean values separately for diabetic and non-diabetic groups
    mean_values_diabetic = train_diabetic[numerical_features].mean()
    mean_values_non_diabetic = train_non_diabetic[numerical_features].mean()

    # Impute missing values for diabetic patients using the diabetic mean
    train.loc[train['Outcome'] == 1, numerical_features] = \
        train.loc[train['Outcome'] == 1, numerical_features].fillna(mean_values_diabetic)
    test.loc[test['Outcome'] == 1, numerical_features] = \
        test.loc[test['Outcome'] == 1, numerical_features].fillna(mean_values_diabetic)

    # Impute missing values for non-diabetic patients using the non-diabetic mean
    train.loc[train['Outcome'] == 0, numerical_features] = \
        train.loc[train['Outcome'] == 0, numerical_features].fillna(mean_values_non_diabetic)
